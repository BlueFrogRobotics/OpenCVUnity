...
using System;using System.Runtime.InteropServices;namespace OpenCVUnity{
	...
	/// <summary>class CV_EXPORTS_W Algorithm <code></summary>
	///
	/// <p>// C++ code:</p>
	///
	///
	...
	///
	/// <a href="http://docs.opencv.org/modules/core/doc/basic_structures.html#algorithm">org.opencv.core.Algorithm</a>
	public class Algorithm : DisposableOpenCVObject
    {
            ...
            try
            {
                ...
                if (IsEnabledDispose)
                {
                    if (nativeObj != IntPtr.Zero)
                        opencvunity_core_Algorithm_delete(nativeObj);
        ...
        protected Algorithm()
        {
            nativeObj = opencvunity_core_Algorithm_algorithm();
        }

        protected void Clear()
        {
            opencvunity_core_Algorithm_clear_0(nativeObj);
        }        protected void Save(string filename)
        {
            opencvunity_core_Algorithm_save_0(nativeObj, filename);
        }        protected string GetDefaultName()
        {
            return opencvunity_core_Algorithm_getDefaultName_0(nativeObj);
        }

        [DllImport("opencvunity", CallingConvention = CallingConvention.Cdecl)]
        private static extern void opencvunity_core_Algorithm_delete(IntPtr nativeObj);

        [DllImport("opencvunity", CallingConvention = CallingConvention.Cdecl)]
        private static extern IntPtr opencvunity_core_Algorithm_algorithm();

        [DllImport("opencvunity", CallingConvention = CallingConvention.Cdecl)]
        private static extern void opencvunity_core_Algorithm_clear_0(IntPtr nativeObj);

        [DllImport("opencvunity", CallingConvention = CallingConvention.Cdecl)]
        private static extern void opencvunity_core_Algorithm_save_0(IntPtr nativeObj, string filename);

        [DllImport("opencvunity", CallingConvention = CallingConvention.Cdecl)]
        private static extern string opencvunity_core_Algorithm_getDefaultName_0(IntPtr nativeObj);
