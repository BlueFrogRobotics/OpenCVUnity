...
template<typename T>
void matToTexture(Mat* mat, uchar* text, T conv) {

	if (mat->channels() == 1) {
		int k = 0;
		size_t sizeTex = mat->total() /* * sizeof(uchar) */ * 4;
		for (size_t i = 0; i < sizeTex; i += 4) {
			text[i] = (T)mat->data[k] * conv;
			text[i + 1] = (T)mat->data[k] * conv;
			text[i + 2] = (T)mat->data[k++] * conv;
			text[i + 3] = 255;
		}
	}
		...
		for (size_t i = 0; i < sizeTex; i += 4) {
			text[i] = (T)mat->data[k] * conv;
			text[i + 1] = (T)mat->data[k + 1] * conv;
			text[i + 2] = (T)mat->data[k + 2] * conv;
			text[i + 3] = 255;
			k += 3;
		}
	}
		...
		for (size_t i = 0; i < sizeTex; ++i)
			text[i] = mat->data[i];
		//size_t sizeText = mat->total() /* * sizeof(uchar) */ * 4;
		//size_t sizeConv = sizeof(T) / 8;
		//size_t col = 0;
		//size_t row = 0;
		//size_t newCol = 0;
		//size_t newRow = 0;
		...
		//	col = (i / 4) % mat->width();
		//	row = (i / 4) / mat->width();
		//	newCol = (col + sizeConv) % sizeConv;
		//	newRow = (row + sizeConv) % sizeConv;
		//	text[4 * sizeConv * row] = mat->data[i];
	}
}
...
template<typename T>
void textureToMat(uchar* text, Mat* mat, T conv) {
	if (mat->channels() == 1) {
		int k = 0;
		size_t sizeMat = mat->total() /* * sizeof(uchar) */;
		for (size_t i = 0; i < sizeMat; ++i) {
			mat->data[i] = (T)((T)text[k] / conv);
			k += 4;
		}
	}
	else if (mat->channels() == 3) {
		int k = 0;
		size_t sizeMat = mat->total() /* * sizeof(uchar) */ * 3;
		for (size_t i = 0; i < sizeMat; i += 3) {
			mat->data[i] = (T)((T)text[k] / conv);
			mat->data[i + 1] = (T)((T)text[k + 1] / conv);
			mat->data[i + 2] = (T)((T)text[k + 2] / conv);
			k += 4;
		}
	}
	else if (mat->channels() == 4) {
		size_t sizeMat = mat->total() /* * sizeof(uchar) */ * 4;
		/*size_t sizeConv = sizeof(T) / 8;
		size_t col = 0;
		size_t row = 0;
		size_t newCol = 0;
		size_t newRow = 0;
		for (size_t i = 0; i < sizeMat; i += 4) {
			col = (i / 4) % mat->cols;
			row = (i / 4) / mat->cols;
			newCol = (col + sizeConv - 1) % sizeConv;
			newRow = (row + sizeConv - 1) % sizeConv;
			mat->data[newRow + newCol] = text[i];
			mat->data[newRow + newCol + 1] = text[i + 1];
			mat->data[newRow + newCol + 2] = text[i + 2];
			mat->data[newRow + newCol + 3] = text[i + 3];
		}*/
		for (size_t i = 0; i < sizeMat; ++i)
			mat->data[i] = text[i];
	}
